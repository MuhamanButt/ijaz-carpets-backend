...
@api_view(['GET'])
def get_products_by_size(request):
    size = request.query_params.get('size')
    
    if not size:
        return Response({'message': 'Size parameter is required'}, status=status.HTTP_400_BAD_REQUEST)
    
    products = list(product_collection.find({"sizes_available": size}))
    
    if products:
        serializer = ProductSerializer(products, many=True)
        return Response(serializer.data, status=status.HTTP_200_OK)
    
    return Response({'message': 'No products found for the given size'}, status=status.HTTP_404_NOT_FOUND)
...
def get_products_by_name(request):
    ...
    suggestions = list(product_collection.find(query).limit(int(limit)))
...
def get_random_products(request):
    limit = request.query_params.get('limit', '10')  # Default limit is 10 if not provided

    try:
        limit = int(limit)
    except ValueError:
        return Response({'message': 'Invalid limit value'}, status=status.HTTP_400_BAD_REQUEST)

    # Aggregate query to get a random sample of products
    pipeline = [
        {"$match": {"hide": False}},  # Ensure products are not hidden
        {"$sample": {"size": limit}}  # Randomly sample 'limit' number of documents
    ]

    # Fetch products
    suggestions = list(product_collection.aggregate(pipeline))
